...
const categoryById = async (req: express.Request, res: express.Response) => {
    try {
        const category = await productCategoryModel.getCategoriesById(req.params.id);
        res.json(category);
    } catch (err) {
        res.status(400);
        res.json(err);
...
const updateCategory = async (req: express.Request, res: express.Response) => {
    ...
    try {
        const updatedCategory = await productCategoryModel.updateCategory(req.params.id, categoryUpdates);
...
const deleteCategory = async (req: express.Request, res: express.Response) => {
    try {
        const deletedCategory = await productCategoryModel.deleteCategory(req.params.id);
