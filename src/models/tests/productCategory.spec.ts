...
describe("Products Categories Model defination", () => {
    ...
    describe("Products Categories Routes", () => {
        it("Get all Categories endpoint", async () => {
            const res = await req.get("/products/categories").set("Authorization", `bearer ${token}`);
            expect(res.status).toBe(200);
        });
        it("Get Category by id route ", async () => {
            const res = await req.get("/products/categories/1").set("Authorization", `bearer ${token}`);
            expect(res.status).toBe(200);
        });
        it("Create categories route", async () => {
            ...
            const res = await req.post("/products/categories").send(newCategory).set("Authorization", `bearer ${token}`);
            expect(res.status).toBe(201);
        });
        it("update categories route ", async () => {
            const res = await req.put("/products/categories/1").send([{ "categoryName": "Metal" }]).set("Authorization", `bearer ${token}`);
            expect(res.status).toBe(200);
        });
        it("delete categories route", async () => {
            const res = await req.delete("/products/categories/3").set("Authorization", `bearer ${token}`);
            expect(res.status).toBe(200);
        });
    })
