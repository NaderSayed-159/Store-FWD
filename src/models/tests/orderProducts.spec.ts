import { AddedProduct, OrderProductModel } from "../orderProductModel";
...
describe("Orders Products Model defination", () => {
  ...
  describe("orders products actions", () => {
    ...
    it("update product in cart", async () => {
      await productModel.createProduct(product);
      const productInCart = await orderProductModel.updateCartProduct(
        10,
        "1",
        "2"
      );
  ...
  describe("Orders PRoducts routes", () => {
    ...
    it("add product to cart route", async () => {
      const res = await req
        ...
        .send({ order_id: 1, product_id: "2", quantity: 5 })
        .set("Authorization", `bearer ${token}`);
      expect(res.status).toBe(200);
    });
